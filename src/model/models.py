...
def resnext(x):
    ...
    # cardinality (basically the branching factor) for ResNeXt as described in the paper
    cardinality = 4
    ...
    def grouped_convolution(y, num_channels, _strides):

        assert not num_channels % cardinality
        _d = num_channels // cardinality
    ...
    def resnextblock(y, num_channels_in, num_channels_out, _strides = (1, 1), _project_shortcut = False):

        shortcut = y

        y = Conv2D(num_channels_in, (1, 1), strides = (1, 1), padding = 'same')(y)
        y = add_common_layers(y)

        y = grouped_convolution(y, num_channels_in, _strides = _strides)
        y = add_common_layers(y)

        y = Conv2D(num_channels_out, (1, 1), strides = (1, 1), padding = 'same')(y)
        y = BatchNormalization()(y)

        if _project_shortcut or _strides != (1, 1):
            shortcut = Conv2D(num_channels_out, (1, 1), strides = _strides, padding = 'same')(shortcut)
    ...
    for i in range(3):
        ...
        x = resnextblock(x, 128, 256, _project_shortcut = project_shortcut)

    # conv 3
    for i in range(4):
        strides = (2, 2) if i == 0 else (1, 1)
        x = resnextblock(x, 256, 512, _strides = strides)

    # conv 4
    for i in range(6):
        strides = (2, 2) if i == 0 else (1, 1)
        x = resnextblock(x, 512, 1024, _strides = strides)

    # conv 5
    for i in range(3):
        strides = (2, 2) if i == 0 else (1, 1)
        x = resnextblock(x, 1024, 2048, _strides = strides)
